...
{if $g.numimages != 0 }
    <div class="gallery-index-item">
        ...
        {if $g.keyimages}<div class="keyimages">{foreach from=$g.keyimages item=i}<a href="{$g.url}"><img class="float-left" src="{$SITEURL}/images/{if $g.thumbsize}{$g.thumbsize}{else}s100{/if}/{$i}" alt="" /></a>{/foreach}</div>{/if}
